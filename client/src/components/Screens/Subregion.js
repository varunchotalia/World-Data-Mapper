...
const Subregion = (props) => {
    ...
    //const activeMap = this.props.location.state.activeMap;
 ...
 //   const [regionList, setRegionList] = useState({});
    ...
    const addRegion = async () => {
		...
		let regionID = newRegion._id;
        console.log("regionid is "+regionID);
        let transaction = new UpdateRegions_Transaction(activeMap._id, regionID, newRegion, opcode, AddRegion, DeleteRegion);
		props.tps.addTransaction(transaction);
		tpsRedo();
        //const { data } = await AddRegion({ variables: { region: newRegion, _id: activeRegionID, index: index}});
        setIndex(index+1);	
        // activeRegion.children.push(regionID);
        // const { data } = await UpdateRegion({ variables: { _id: activeMap._id, regionId: activeRegion._id, field: children, value: activeRegion.children}});
	};
    ...
    const deleteRegion = async (region, index) => {
		...
		let transaction = new UpdateRegions_Transaction(activeMap._id, regionID, regionToDelete, opcode, AddRegion, DeleteRegion, index);
		props.tps.addTransaction(transaction);
		tpsRedo();
    }
    ...
    const sort = (criteria) => {
		...
		let transaction = new SortRegions_Transaction(activeMap._id, criteria, prevSortRule, sortRegions);
    ...
    const handleRegionClick = () =>{
        props.tps.clearAllTransactions();
        let temp = maps.find(list => list._id === activeRegion.parent);
        ...
        <WLayout wLayout="header-lside">
            ...
            {    
              ...
              <WRow style= {{fontSize: "25px", paddingLeft: "246px", paddingTop: "50px"}}>
                ...
                <WCol size= "2" style={{color:"blue", cursor:"pointer"}} onClick={handleRegionClick}>{" "+activeRegion.name}</WCol>
              </WRow>
            }
            </WLHeader>
            <WCard wLayout="header-content-footer" style={{ width: "1025px", height: "550px", position: "fixed", left: "16%", top: "20%", overflowY: "auto"}} raised className="example-layout-labels">
                ...
                <WCContent style={{ backgroundColor: "grey", overflowY: "auto" }}>
                 { activeMap &&
                  <RegionList activeList={activeMap} editRegion={editRegion} deleteRegion={deleteRegion} regionList={regionList} tps={props.tps}
                  activeRegionID={activeRegionID} reloadList={reloadList} />
                 }
                  </WCContent>
